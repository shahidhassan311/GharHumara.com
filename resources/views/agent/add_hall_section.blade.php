                                ...
                                <div class="source-preview-wrapper">
                                    <div class="preview">
                                        <div class="preview-elements">
                                            ...
                                            <form action="{{ ($update) ? url('/agent_hall_section_update') : url('/agent_add_hall_section_store')}}" method="post" enctype="multipart/form-data">
                                                {{ csrf_field() }}
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select class="form-control" name="hall_name" id="exampleFormControlSelect1">
                                                        @foreach ($hall as $key => $halls)
                                                            <option value="{{ $halls->id }}" {{($update) ? $hall_section->halls_id : $halls->id ? 'selected="selected"' : ''}}>{{ $halls->hall_name }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="form-group">
                                                ...
                                                @if($update)
                                                    <div class="form-group">
                                                        ...
                                                        <input id="input-24" class="btn-success" name="images[]" type="file" multiple style="background-color: #c6ac55">
                                                    </div>
                                                @else
                                                    <div class="form-group">
                                                        <label for="input-24">Add Cover Picture</label>
                                            ...
                                            @if($update)
                                                ...
                                                <a href="/hall_section_image_delete/{{$hall_section_images->id}}/{{ Request::segment(2) }}">
                                                    <button class="btn btn-secondary">DELETE</button>
                                                </a>
                                                <img src="{{ URL::to('/') }}/uploads/{{ $hall_section_images->images }}" height="200" width="300" alt="">
                                                @endforeach
                                            @else
                                            @endif
                                        </div>
